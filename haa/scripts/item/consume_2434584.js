...
function action(mode, type, selection) {
    if (cm.getPlayer().getKeyValue(20190211, "karuta_1") < 0) {
        cm.getPlayer().setKeyValue(20190211, "karuta_1", 0);
    }
    itemlist = [1042254, 1042255, 1042256, 1042257, 1042258];
    if (mode == -1 || mode == 0) {
        cm.dispose();
        return;
    }
    ...
    if (status == 0) {
        ...
        if (cm.itemQuantity(2434584) < 5) {
            cm.sendOk("조각이 모자랍니다");
            cm.dispose();
            return;
        }
        talk = "#b#e찬스타임 게이지:#r"+cm.getPlayer().getKeyValue(20190211, "karuta_1")+"%#k#n\r\n\r\n"
        if (cm.getPlayer().getKeyValue(20190211, "karuta_1") >= 100) {
            talk+= "#r#e찬스타임 발동!#k#n\r\n"
        ...
        cm.getPlayer().setKeyValue(20190211, "karuta_1", cm.getPlayer().getKeyValue(20190211, "karuta_1") + Packages.server.Randomizer.rand(7,11));
        if (cm.getPlayer().getKeyValue(20190211, "karuta_1") >= 100) {
            cm.getPlayer().setKeyValue(20190211, "karuta_1", 100)
        }
        cm.gainItem(itemlist[selected], 1);
        cm.gainItem(2434584, -5);
        cm.dispose();
    }
}
