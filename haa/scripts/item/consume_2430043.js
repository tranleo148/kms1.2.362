...
function action (mode, type, selection) {
    ...
    if (status == 0) {
        var count = 0;
        var say = "#fs11##b+15강화권#k을 사용할 장비를 선택 해주세요\r\n\r\n";
      ...
      //  "#L1##b" + ConvertNumber(keep) + "메소를 사용하여 파괴, 미끄러짐을 방지 하겠습니다.#l\r\n" +
        ...
        say += "#fs 11#";
        say += "강화 : #b0강 -> 15강#k\r\n";
        say += "강화 성공 시 #b올스탯 + 232 공마 + 195#k 증가\r\n";
        ...
        if (cm.getInventory(1).getItem(slot) != null) {
            ...
            if (rand >= 0 && rand <= items[getAddEnhance(item) + 1][1]) {
                item.setStr(item.getStr() + 232);
                item.setDex(item.getDex() + 232);
                item.setInt(item.getInt() + 232);
                item.setLuk(item.getLuk() + 232);
                item.setWatk(item.getWatk() + 195);
                item.setMatk(item.getMatk() + 195);
