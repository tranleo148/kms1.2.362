...
var hairs = [39250, 39500, 39080, 39150, 39160, 39040, 39190];
...
function action(mode, type, sel) {
	...
	if (status == 0) {
		var msg = "#fUI/GuildMark.img/Mark/Letter/00005014/4##fs11# #eDEEZ 스페셜 헤어쿠폰(여) D#n를 획득하셨군요! (현재 7종)\r\n#fUI/GuildMark.img/Mark/Letter/00005014/4# 헤어쿠폰 사용 전 주의사항이 있으니 꼭 읽어주세요."+enter+enter;
		...
		msg += "#L2##e#bDEEZ 스페셜 헤어 랜덤쿠폰(여)D를 사용하겠습니다.";
		cm.sendSimple(msg);
	} else if (status == 1) {
		seld = sel;
		switch (sel) {
			...
			case 2:
				...
				cm.sendYesNo("정말 DEEZ 스페셜 헤어쿠폰 D를 사용하시겠어요?\r\n7종의 헤어 중 랜덤한 하나의 헤어가 적용됩니다.");
 			break;
		}
	} else if (status == 2) {
		if (cm.itemQuantity(itemid) < price) {
		...
		if (color > 0 || cm.getPlayer().getBaseColor() != -1 || cm.getPlayer().getAddColor() != 0) {
			...
			cm.setAvatar(2430024, cm.getPlayer().getHair() - color);
		}
		ind = Randomizer.rand(0, (hairs.length - 1));
		cm.gainItem(itemid, -price);
		cm.setAvatar(2430024, hairs[ind]);
