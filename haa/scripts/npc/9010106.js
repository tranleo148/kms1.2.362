...
//보상받았음?
function overlab_unionbosang(name) {
	var c = DatabaseConnection.getConnection();
	var con = c.prepareStatement("SELECT * FROM unionbosang WHERE name LIKE '"+name+"%'").executeQuery();
	overlab = true;
...
//보상받기
function join_union(name) {
	var con = DatabaseConnection.getConnection();
	var insert = con.prepareStatement("INSERT INTO unionbosang(name) VALUES(?)");
...
function getRequire() {
	switch (main) {
		...
		break;
		case 2:
			require = sub == 1 ? 430 : sub == 2 ? 1 : sub == 3 ? 470 : sub == 4 ? 490 : sub == 5 ? 510 : -1;
		break;
		case 3:
	...
	if (status == 0) {
		if (cm.getPlayer().getMapId() == outmap || cm.getPlayer().getMapId() == outmap2) {
			...
			msg += "#L3#<메이플 유니온에 대해 설명을 듣는다.>"+enter;
                                            msg += "#L99#<유니온 8000달성 보상을 받는다.>"+enter;
			//msg += "#L4#<주간 획득 코인 랭킹>";
